...
class contentbuilder_com_breezingforms{
    ...
    function __construct($id){
        ...
        $db->setQuery("Select * From #__facileforms_forms Where id = ".intval($id)." And published = 1 Order By `ordering`");
    ...
    public function getListRecords(
        ...
        foreach($elements As $element){
            ...
            if(!in_array($element['id'], $ids)){
                ...
                if($element['type'] == 'Radio Button' || $element['type'] == 'Checkbox'){
                    ...
                    $bottom .= $cast_open."GROUP_CONCAT( DISTINCT ( Case When s.`name` = '{$element['name']}' Then s.`value` End ) Order By s.`id` SEPARATOR ', ' )".$cast_close." As `col{$element['id']}`,";
                }else{
                    $bottom .= $cast_open."GROUP_CONCAT( DISTINCT ( Case When s.`element` = '{$element['id']}' Then s.`value` End ) Order By s.`id` SEPARATOR ', ' )".$cast_close." As `col{$element['id']}`,";
                }
            }
        }
        ...
        foreach($ids As $id){
            ...
            if(!isset($act_as_registration[$id])){
                ...
                if(isset($radio_buttons[$id])){
                    $selectors .= $cast_open."GROUP_CONCAT( DISTINCT ( Case When s.`name` = '{$radio_buttons[$id]}' Then s.`value` End ) Order By s.`id` SEPARATOR ', ' )".$cast_close." As `col$id`,";
                }else{
                    $selectors .= $cast_open."GROUP_CONCAT( DISTINCT ( Case When s.`element` = '".intval($id)."' Then s.`value` End ) Order By s.`id` SEPARATOR ', ' )".$cast_close." As `col$id`,";
                }

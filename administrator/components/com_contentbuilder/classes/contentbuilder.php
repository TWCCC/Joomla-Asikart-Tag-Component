...
.cbRatingImage{ 
    ...
    background-repeat: no-repeat;
        ...
        if($rating_allowed){
            if(JFactory::getApplication()->isSite()){
                $rating_link = JURI::root(true) . (JFactory::getApplication()->isAdmin() ? '/administrator' : (JRequest::getCmd('lang','') && JFactory::getConfig()->getValue('config.sef') && JFactory::getConfig()->getValue('config.sef_rewrite') ? '/'.JRequest::getCmd('lang','') : '') ).'/?option=com_contentbuilder&lang='.$lang.'&controller=ajax&format=raw&subject=rating&id='.$form_id.'&record_id='.$record_id;
            }else{
    ...
    public static function getEmailTemplate($contentbuilder_form_id, $record_id, array $record, array $elements_allowed, $isAdmin){
        ...
        $hash = md5($contentbuilder_form_id . $record . implode(',',$elements_allowed));
        ...
        if(is_array($result) && $result['editable_template']){
            ...
            foreach($items As $key => $item){
                ...
                if(is_array($element)){
                    ...
                    switch($element['type']){
                      ...
                      case 'select':
                           ...
                           if($form->isGroup($item['id'])){
                               ...
                               $group = explode($sep, $failed_values !== null && isset($failed_values[$element['reference_id']]) && is_array($failed_values[$element['reference_id']]) ? implode($sep, $failed_values[$element['reference_id']]) : ( $hasRecords ? $item['value'] : $element['default_value'] ) );
                               $the_item = '<input name="cb_'.$item['id'].'[]" type="hidden" value="cbGroupMark"/>';
                               $the_item .= '<div class="cbFormField cbSelectField"><select id="cb_'.$item['id'].'" '.($options->length ? 'style="width:'.$options->length.';" ' : '').'name="cb_'.$item['id'].'[]"'.($multi ? ' multiple="multiple"' : '').'>';
                               foreach($groupdef As $value => $label){
                                  $checked = '';
                                  foreach($group As $selected_value){
                                      if(trim($value) == trim($selected_value)){
                     ...
                     case 'captcha':
                         ...
                         $the_item .= '<div>';
                         $the_item .= '<input autocomplete="off" id="cb_'.$item['id'].'" name="cb_'.$item['id'].'" style="width:228px;" type="text" maxlength="12" />';
                         $the_item .= '<img style="cursor: pointer;" onclick="document.getElementById(\'cbCaptcha\').src = \''.$captcha_url.'?\' + Math.random(); blur(); return false" border="0" width="15" height="18" alt="refresh" src="'.JURI::root(true).'/components/com_contentbuilder/images/securimage/images/refresh.png"/>';
                         $the_item .= '</div>';
                         $the_item .= '</div>';
                         break;
                     case 'calendar':
                    ...
                    if($the_item){
                        $template = str_replace('{'.$key.':label}','<label '.($element['hint'] ? 'class="editlinktip hasTip" title="'.htmlentities($item['label'],ENT_QUOTES,'UTF-8').'::'.$element['hint'].'" ' : '').'for="cb_'.$item['id'].'">'.$item['label'].$asterisk.($element['hint'] ? ' <img style="cursor: pointer;" src="'.JURI::root(true).'/components/com_contentbuilder/images/icon_info.png" border="0"/>' : '').'</label>', $template);
    ...
    public static function createArticle($contentbuilder_form_id, $record_id, array $record, array $elements_allowed, $title_field = '', $metadata = null, $config = array(), $full = false, $limited_options = true, $menu_cat_id = null){
        ...
        if(!$is15 && $full){
            ...
            if($form['article_record_impact_publish'] && isset($config['publish_up']) && $config['publish_up'] != $publish_up){
               ...
               $db->setQuery("Update #__contentbuilder_records Set ".(strtotime($config['publish_up'] == '0000-00-00 00:00:00' ? '1976-03-07 22:10:00' : $config['publish_up']) >= strtotime($_now->toMySQL()) ? 'published = 0, is_future = 1, ' : '')." publish_up = ".$db->Quote($config['publish_up'])." Where `type` = ".$db->Quote($form['type'])." And reference_id = ".$db->Quote($form['reference_id'])." And record_id = " . $db->Quote($record_id));
               $db->query();
            }
            ...
            if($form['article_record_impact_publish'] && isset($config['publish_down']) && $config['publish_down'] != $publish_down){
                $db->setQuery("Update #__contentbuilder_records Set ".(strtotime($config['publish_down'] == '0000-00-00 00:00:00' ? '1976-03-07 22:10:00' : $config['publish_down'] ) <= strtotime($_now->toMySQL()) ? 'published = 0,' : '')."  publish_down = ".$db->Quote($config['publish_down'])." Where `type` = ".$db->Quote($form['type'])." And reference_id = ".$db->Quote($form['reference_id'])." And record_id = " . $db->Quote($record_id));
                $db->query();
            }
        ...
        if(!$article){
            ...
            $article = $db->insertid();
            $datenow = JFactory::getDate();
            $db->setQuery("Insert Into #__contentbuilder_articles (`type`,`reference_id`,`last_update`,`article_id`,`record_id`,`form_id`) Values (".$db->Quote($form['type']).",".$db->Quote($form['reference_id']).",".$db->Quote($datenow->toMySQL()).",$article,".$db->Quote($record_id).",".intval($contentbuilder_form_id).")");
            $db->query();
        ...
        // existing, update
            ...
            $datenow = JFactory::getDate();
            $modified = $metadata->modified ? $metadata->modified : $datenow->toMySQL();
            ...
            $db->setQuery("Update #__contentbuilder_articles Set `last_update` = ".$db->Quote($datenow->toMySQL())." Where `type` = ".$db->Quote($form['type'])." And form_id = ".intval($contentbuilder_form_id)." And reference_id = ".$db->Quote($form['reference_id'])." And record_id = ".$db->Quote($record_id));
            $db->query();
        }
    ...
    public static function setPermissions($form_id, $record_id = 0, $suffix = ''){
        ...
        $db->setQuery("
            ...
            From 
                ...
                On ( contentbuilder_records.`type` = ".$db->Quote($type['type'])." And contentbuilder_records.reference_id = forms.reference_id And contentbuilder_records.record_id = ".$db->Quote($record_id)." )
                " : '')."
            Where 
        ...
        if(version_compare($version->getShortVersion(), '1.6', '<')){
            ...
            if(!isset($permissions['own'.$suffix])){
                ...
                if(JFactory::getUser()->get('id',0)){
                    $groups = JFactory::getUser()->get('groups',array());
                }else{

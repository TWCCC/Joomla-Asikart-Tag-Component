...
<form action="" method="get" name="adminForm" id="adminForm" onsubmit="return false">
    ...
    ?>
    <?php
    if($this->export_xls && !$this->show_page_heading):
    ?>
    <a href="<?php echo JRoute::_('index.php?option=com_contentbuilder&controller=export&id='.JRequest::getInt('id',0).'&type=xls&format=raw&tmpl=component'); ?>"><img border="0" src="components/com_contentbuilder/images/xls.png" alt="Export"/></a>
    <?php
    endif;
    ?>
    ...
    <thead>
        <?php 
        if($new_allowed || $delete_allowed || $state_allowed || $publish_allowed || $language_allowed){
        ?>
        <tr>
            <td align="right" colspan="1000">
                ...
                if ($new_allowed) {
                ?>
                <button class="button cbButton cbNewButton" onclick="location.href='<?php echo JRoute::_( 'index.php?option=com_contentbuilder&controller=edit&backtolist=1&id='.JRequest::getInt('id', 0).'&record_id=&limitstart='.JRequest::getInt('limitstart',0).'&filter_order='.JRequest::getCmd('filter_order') ); ?>'"><?php echo JText::_('COM_CONTENTBUILDER_NEW'); ?></button>
                <?php
                }
                ?>
                ...
                if ($delete_allowed) {
                ?>
                <button class="button cbButton cbDeleteButton" onclick="contentbuilder_delete();"><?php echo JText::_('COM_CONTENTBUILDER_DELETE'); ?></button>
                <?php
                }
                ?>
                ...
                <button class="button cbButton cbSetButton" onclick="contentbuilder_state();"><?php echo JText::_('COM_CONTENTBUILDER_SET'); ?></button>
                <?php  
                }
                ?>
                ...
                <button class="button cbButton cbSetButton" onclick="contentbuilder_publish();"><?php echo JText::_('COM_CONTENTBUILDER_SET'); ?></button>
                <?php  
                }
                ?>
                ...
                <button class="button cbButton cbSetButton" onclick="contentbuilder_language();"><?php echo JText::_('COM_CONTENTBUILDER_SET'); ?></button>
                <?php
                }
                ?>
            </td>
        ...
        if($this->display_filter || $this->show_records_per_page){
        ?>
        <tr>
            <td align="right" colspan="1000">
        ...
        <tr>
            ...
            ?>
            <<?php echo $th; ?> class="sectiontableheader" width="5">
    ...
    for ($i=0; $i < $n; $i++)
    {
        ...
        foreach($row As $key => $value){
            if(!$first && $key != 'colRecord'){
                $first = html_entity_decode(cbinternal($value), ENT_QUOTES, 'UTF-8');
            }
            // filtering out disallowed columns
            if(str_replace('col','',$key) == $this->title_field ){
                $label = html_entity_decode(cbinternal($value), ENT_QUOTES, 'UTF-8');
                break;
            }
        }
        ...
        $link = JRoute::_( 'index.php?option=com_contentbuilder&title='.contentbuilder::stringURLUnicodeSlug($this->slug.'-'.$this->slug2.$label).'&controller=details&id='.$this->form_id.'&record_id='.$row->colRecord.'&Itemid='.JRequest::getInt('Itemid',0).'&limitstart='.JRequest::getInt('limitstart',0).'&filter_order='.JRequest::getCmd('filter_order') );
        $edit_link = JRoute::_( 'index.php?option=com_contentbuilder&controller=edit&backtolist=1&id='.$this->form_id.'&record_id='.$row->colRecord.'&Itemid='.JRequest::getInt('Itemid',0).'&limitstart='.JRequest::getInt('limitstart',0).'&filter_order='.JRequest::getCmd('filter_order') );
        $publish_link = JRoute::_( 'index.php?option=com_contentbuilder&controller=edit&task=publish&backtolist=1&id='.$this->form_id.'&list_publish=1&cid[]='.$row->colRecord.'&Itemid='.JRequest::getInt('Itemid',0).'&limitstart='.JRequest::getInt('limitstart',0).'&filter_order='.JRequest::getCmd('filter_order') );
        $unpublish_link = JRoute::_( 'index.php?option=com_contentbuilder&controller=edit&task=publish&backtolist=1&id='.$this->form_id.'&list_publish=0&cid[]='.$row->colRecord.'&Itemid='.JRequest::getInt('Itemid',0).'&limitstart='.JRequest::getInt('limitstart',0).'&filter_order='.JRequest::getCmd('filter_order') );
        ...
        <tr class="<?php echo "row$k"; ?>">
            ...
            if($this->list_state){
            ?>
            <td style="background-color: #<?php echo isset($this->state_colors[$row->colRecord]) ? $this->state_colors[$row->colRecord] : 'cccccc'; ?>;">
